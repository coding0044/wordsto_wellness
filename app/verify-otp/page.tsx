...
function VerifyOTPForm() {
  ...
  useEffect(() => {
    if (!email) {
      ...
      if (storedEmail) {
        router.push(`${Routes.auth.verifyOtp}?email=${encodeURIComponent(storedEmail)}`);
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.resetToken) {
        ...
        setTimeout(() => {
          router.push(`${Routes.auth.resetPassword}?token=${data.resetToken}`);
        }, 500);
      } else {
        setMessage('');
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const res = await fetch(ApiRoutes.auth.resendOtp, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
