...
function CategoryCard({ category }) {
  ...
  return (
    <Link href={`/categories/${category.slug || category._id}`} className="group block bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md hover:border-sky-200 transition-all duration-200">
      <div className="flex items-start justify-between mb-4">
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-sky-100 to-teal-100 flex items-center justify-center">
      ...
      <div className="flex items-center justify-between text-xs text-gray-400 mt-4 pt-2 border-t border-gray-100">
        ...
        <span className="group-hover:text-sky-600 transition-colors">Explore →</span>
      </div>
    </Link>
  );
}
...
function DashboardContent() {
  ...
  // Fetch categories - NO SORTING, display in database order
  useEffect(() => {
    async function fetchCategories() {
      try {
        ...
        if (response.ok) {
          ...
          // Display exactly as from database - NO sorting applied
  ...
  const preferredCategoryOrder = [
    '6a0c6aecbcdf1dcbba76bf01', // Relationship Issues
    '6a0c6aecbcdf1dcbba76bf02', // Physical Emotional And Behavioral Health Issues
    '6a0c6aecbcdf1dcbba76bf03', // Victims Of Abuse
    '6a0c6aecbcdf1dcbba76bf04', // Parent Teen And Family Communication Issues
  ];
  ...
  function reorderCategories(list, order) {
    ...
    // append remaining categories in original DB order
    list.forEach((c) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-sky-50 to-teal-50">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        {/* Categories Section - Displayed in database order (NO sorting) */}
